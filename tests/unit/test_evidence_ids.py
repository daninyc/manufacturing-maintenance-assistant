...
def test_pdf_newline_is_returned_unchanged():
    ...
    result = resolve_selection(SelectedEvidenceIds(supported=True, evidence_ids=[next(iter(candidates))]), candidates)
...
def test_nonadjacent_sentences_remain_separate():
    ...
    result = resolve_selection(SelectedEvidenceIds(supported=True, evidence_ids=[keys[0], keys[2]]), candidates)
...
def test_candidate_bounds_preserve_decimals_and_condition():
    ...
    quotes = list(candidates.values())
    assert quotes[0].excerpt == "Repeatability 0.03 mm."
    assert quotes[1].excerpt == "Only under rated conditions."
...
def test_invalid_selection_is_not_partially_accepted(ids, reason):
    ...
    ids = [next(iter(candidates)) if key == "E001" else key for key in ids]
...
def test_duplicate_ids_are_removed():
    ...
    result = resolve_selection(SelectedEvidenceIds(supported=True, evidence_ids=[key, key]), candidates)
...
def test_online_contract_resolves_ids_not_model_text(monkeypatch):
    ...
    class Response:
        ...
        def read(self, size=-1):
            ...
            return json.dumps({"choices": [{"message": {"content":
                json.dumps({"supported": True, "evidence_ids": [context[0]["evidence_id"]]})}}]}).encode()
    ...
    context = json.loads(captured["messages"][1]["content"])["context"]
    assert context[0]["evidence_id"].endswith(":E001")
    assert trace["model_selection"]["evidence_ids"] == [context[0]["evidence_id"]]


def test_previous_request_ids_cannot_select_current_evidence():
    first = build_candidates([chunk("相同正文。")])
    second = build_candidates([chunk("相同正文。")])
    assert set(first).isdisjoint(second)
    replay = SelectedEvidenceIds(supported=True, evidence_ids=list(first))
    with pytest.raises(EvidenceError, match="UNKNOWN_EVIDENCE_ID"):
        resolve_selection(replay, second)
